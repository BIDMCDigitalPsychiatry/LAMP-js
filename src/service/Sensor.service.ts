...
export class SensorService {
  ...
  public async allByParticipant(
    ...
    return (
      await Fetch.get<{ data: any[] }>(
        `/participant/${participantId}/sensor?ignore_binary=${ignore_binary}`,
        this.configuration
      )
    ).data.map((x) => Object.assign(new Sensor(), x))
  }
  ...
  public async allByStudy(studyId: Identifier, transform?: string, ignore_binary?: boolean): Promise<Sensor[]> {
    ...
    return (
      await Fetch.get<{ data: any[] }>(`/study/${studyId}/sensor?ignore_binary=${ignore_binary}`, this.configuration)
    ).data.map((x) => Object.assign(new Sensor(), x))
  }
  ...
  public async view(sensorId: Identifier, transform?: string, ignore_binary?: boolean): Promise<Sensor> {
    ...
    return (
      await Fetch.get<{ data: any[] }>(`/sensor/${sensorId}?ignore_binary=${ignore_binary}`, this.configuration)
    ).data.map((x) => Object.assign(new Sensor(), x))[0]
  }
}
