...
export class SensorService {
     ...
     */
    public async all(): Promise<Sensor[]> {
        if (this.configuration.base === 'https://demo.lamp.digital') { // DEMO
            ...
            return Promise.resolve(Demo.Sensor.map(x => Object.assign(new Sensor(), x)))
        }
        return (await Fetch.get<{data: any[]}>(`/sensor`, this.configuration)).data.map(x => Object.assign(new Sensor(), x))
    }
     ...
     * @param participantId 
     */
    public async allByParticipant(participantId: Identifier): Promise<Sensor[]> {
        ...
        if (this.configuration.base === 'https://demo.lamp.digital') { // DEMO
            ...
            if (Demo.Participant.filter(x => x['id'] === participantId).length > 0) {
                return Promise.resolve(Demo.Sensor.filter(x => Demo.Participant.filter(y => y['id'] === participantId).map(y => y['#parent']).includes(x['#parent'])).map(x => Object.assign(new Sensor(), x)))
            } else {
                return Promise.resolve({ 'error': '404.not-found' } as any)
            }
        }
        return (await Fetch.get<{data: any[]}>(`/participant/${participantId}/sensor`, this.configuration)).data.map(x => Object.assign(new Sensor(), x))
    }
     ...
     * @param researcherId 
     */
    public async allByResearcher(researcherId: Identifier): Promise<Sensor[]> {
        ...
        if (this.configuration.base === 'https://demo.lamp.digital') { // DEMO
            ...
            if (Demo.Researcher.filter(x => x['id'] === researcherId).length > 0) {
                return Promise.resolve(Demo.Sensor.filter(x => Demo.Study.filter(y => y['#parent'] === researcherId).map(y => y['id']).includes(x['#parent'])).map(x => Object.assign(new Sensor(), x)))
            } else {
                return Promise.resolve({ 'error': '404.not-found' } as any)
            }
        }
        return (await Fetch.get<{data: any[]}>(`/researcher/${researcherId}/sensor`, this.configuration)).data.map(x => Object.assign(new Sensor(), x))
    }
     ...
     * @param studyId 
     */
    public async allByStudy(studyId: Identifier): Promise<Sensor[]> {
        ...
        if (this.configuration.base === 'https://demo.lamp.digital') { // DEMO
            ...
            if (Demo.Study.filter(x => x['id'] === studyId).length > 0) {
                return Promise.resolve(Demo.Sensor.filter(x => x['#parent'] === studyId).map(x => Object.assign(new Sensor(), x)))
            } else {
                return Promise.resolve({ 'error': '404.not-found' } as any)
            }
        }
        return (await Fetch.get<{data: any[]}>(`/study/${studyId}/sensor`, this.configuration)).data.map(x => Object.assign(new Sensor(), x))
    }
        ...
        if (this.configuration.base === 'https://demo.lamp.digital') { // DEMO
            ...
            if (Demo.Study.filter(x => x['id'] === studyId).length > 0) {
                let data = {
                    ...
                    "#parent": studyId,
                    ...(sensor as any),
                    "id": "sensor" + Math.random().toString().substring(2, 6),
                }
     ...
     * @param sensorId 
     */
    public async view(sensorId: Identifier): Promise<Sensor> {
        if (sensorId === null || sensorId === undefined)
        ...
        return (await Fetch.get<{data: any[]}>(`/sensor/${sensorId}`, this.configuration)).data.map(x => Object.assign(new Sensor(), x))[0]
    }
}
